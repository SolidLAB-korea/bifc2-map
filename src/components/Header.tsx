...
export default function Header({ selectedTheme, onThemeSelect }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-white/10 bg-primary text-white shadow-panel">
      <div className="app-container flex items-center justify-between gap-2 py-2.5 sm:gap-3 sm:py-4">
        <Link to="/" className="min-w-0 flex-1" aria-label={t("homeAria")}>
          ...
          <h1 className="truncate text-lg font-black leading-tight tracking-normal sm:text-3xl">{t("title")}</h1>
        </Link>
        <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
          ...
          <button
            ...
            className="min-h-10 shrink-0 rounded-lg border border-white/30 px-2.5 py-2 text-xs font-black text-white sm:min-h-11 sm:px-4 sm:py-3 sm:text-sm"
